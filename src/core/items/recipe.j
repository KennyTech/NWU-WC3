library RecipeSYS initializer init uses Table,ItemData,RecetasDisabled,PlayerCore
    ...
    //         ¤                             ¤
        ...
        // game interface)).
        private constant real MAX_RANGE = 150.
...
//PRELOAD-----------------------------------------------------------------
            if PRELOAD_UNIT != null then
                call RemoveItem(UnitAddItemById(PRELOAD_UNIT,result))
                /*set bj_lastCreatedItem=UnitAddItemById(PRELOAD_UNIT,result)
                call SetItemPlayer(bj_lastCreatedItem,Player(2),false)
                call UnitRemoveItem(PRELOAD_UNIT,bj_lastCreatedItem)*/
            endif
//------------------------------------------------------------------------
...
/*    
    function DisassembleItemByRecipe takes item whichItem, unit whichUnit, recipe re returns nothing
        local integer i = 0
        local real x = GetUnitX(whichUnit) // GetItemX(whichItem)
        local real y = GetUnitY(whichUnit) // GetItemY(whichItem)
        ...
        if re.result == GetItemTypeId(whichItem) then
            ...
            //if whichUnit == null then
                loop
                    call SetItemPlayer(CreateItem(re.items[i], x, y),owner,true)
                    set i = i + 1
                    exitwhen i == 6
                endloop
            /*else
...
*/    
    ...
    private function DoCombine takes recipe rec, unit u,player owner, item noncarried returns boolean
        ...
        if D2R(noncarriedId)>0 then
            set noncarriedId = D2R(noncarriedId)
        endif
        ...
        endloop
        set i = 0
        // Primero busquemos por noncarried...
        if noncarried != null then
            loop
                if itemsReq[j] == noncarriedId and noncarried != null and (GetItemPlayer(noncarried) == owner or IsPlayerLeaver(GetItemPlayer(noncarried))) then
        ...
        set j = 0
        loop
            set slot = UnitItemInSlot(u, i)
            set itemInSlot = GetItemTypeId(slot)
            if D2R(itemInSlot)>0 then
                set itemInSlot = D2R(itemInSlot)
            endif
            loop
                if itemsReq[j] == itemInSlot and slot != null and (GetItemPlayer(slot) == owner or IsPlayerLeaver(GetItemPlayer(slot))) then
                    set itemsToCombine[j] = slot
                    set itemsReq[j] = 0
                    exitwhen true
                endif
                set j = j + 1
        ...
        //Create the result.
        set resu = CreateItem(rec.result,GetUnitX(u),GetUnitY(u))
        call SetItemPlayer(resu,owner,true)
        call UnitAddItem(u,resu)
    ...
    function Combine takes item check, unit u returns boolean
        local boolean isCarried 
        local ingIn r
        local integer i 
        
        local item Reemplazo
        
        if check == null then
            return false
        endif
        
        set isCarried = UnitHasItem(u, check)
        
        if D2R(GetItemTypeId(check))>0 then
            set r = ingIn(ING_TABLE[D2R(GetItemTypeId(check))])
        else
            set r = ingIn(ING_TABLE[GetItemTypeId(check)])
        endif
        ...
        loop
            ...
            if r[i].enabled then
                if isCarried then
                    if DoCombine(r[i], u,GetItemPlayer(check), null) then
                        return true
                    endif
                else
                    if DoCombine(r[i], u,GetItemPlayer(check), check) then
                        return true
                    endif
                endif
            endif
    ...
    globals
        constant integer ARTISAN = 'n00G'
        constant integer CIRCLE = 'n01K'
    endglobals
